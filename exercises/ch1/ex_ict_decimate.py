...
def ict_transform(image: np.ndarray, show=False):
    """
    Performs ICT transformation and chroma subsampling using decimate.
    ...
    # Subsample Cb and Cr using decimate
    Cb_sub = decimate(decimate(Cb, 2, axis=0, ftype='fir', zero_phase=True), 2, axis=1, ftype='fir', zero_phase=True)
    Cr_sub = decimate(decimate(Cr, 2, axis=0, ftype='fir', zero_phase=True), 2, axis=1, ftype='fir', zero_phase=True)
...
def reconstruction_pipe(Y, Cb_sub, Cr_sub):
    """
    Reconstructs an RGB image from Y, Cb, Cr using resample for upsampling.
    ...
    # Upsample Cb and Cr using resample to match original shape
    Cb_up = resample(resample(Cb_sub, H, axis=0), W, axis=1)
    Cr_up = resample(resample(Cr_sub, H, axis=0), W, axis=1)
